                ...
                <!-- Jatot Glory IF(condition, value_if_true, value_if_false)-->
                    <tr>
                        ...
                        foreach($count_date_array_glory as $dates3){
                            foreach ($arrayDate as $dates_gloryx){
                                if($dates3['transactions'] == date('Y-m-d',strtotime($dates_gloryx))){ ?> <td style="font-size: 11px; font-weight: bold;"> <?php echo $dates3['total_transactions_glory']; ?></td>  <?php
                                    }elseif($dates3['transactions'] != date('Y-m-d',strtotime($dates_gloryx))){ ?>
...
<!-- MAXIM -->
    ...
    foreach ($arrayDate as $dates2){
        ...
        if($select_transdate_count_maxim == 0){
            ...
            ?>
                <td style="font-size: 11px"> 0 <?php $L; ?></td> 
                <?php }else{ 
                    
                    ?>
                <td style="font-size: 11px"> 25 <?php $L++;?></td> 
                <?php $count = $count + 25;
                }
             } ?>   <td style="font-size: 11px"><?php echo $count ?> </td> <?php $count = 0;
            }
    ?>
                </tr>
                <!-- Jatot maxim -->
                ...
                if($dates3['total_transactions_maxim'] = 0){ ?> 
                    <td style="font-size: 11px; font-weight: bold;">0<?php echo $L;?></td> <?php } }?>
                </tr>
                </table>
                <table style="margin-top: 20px; width: 100%; text-align: center; border-collapse: collapse;">
<!-- POWERLANE -->
...
<!-- Overall -->
                ...
                <tr>
                        ...
                        <?php
                    $count_dates_jatot = "SELECT SUM(jatot) as total FROM (select COUNT(tran_date)*25 as jatot from tbl_trans_logs where tran_date BETWEEN '$fromDate' AND '$toDate' UNION SELECT COUNT(lgbk_date)*25 as jatot FROM logbooksales WHERE lgbk_date BETWEEN '$fromDate' AND '$toDate') t group by NULL";
