...
<body id="totalSalesBody" onload="navFuntion()">
    ...
    <?php
        ...
        if(isset($_POST['sbmtPrint'])){
            ...
            for($x = $fromDate; $x < date_create($toDate)->modify("+1 days")->format("Y-m-d"); $x = date_create($x)->modify("+1 days")->format("Y-m-d")){
                $dateCount++;
                array_push($arrayianDate, $x);
            }

            $_SESSION['period'] = $arrayianDate;
            ...
            ?>
                <script type="text/javascript">
                    window.open('./TSReportPDFtrial.php', '_blank');
                </script>
            <?php
    ...
    <div class="totalSalesContainer">
        <form method="POST">
            <div class="totalSalesControl"><?php echo print_r($arrayianDate); echo $dateCount; ?>
                <span class="fromCon">
